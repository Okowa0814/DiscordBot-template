...
- **誘捕頻道警告訊息**（`cogs/warning_msg.py`）：管理員可用 `/warning-msg` 指令在誘捕頻道發送警告 Embed，提醒一般成員不要在此發言。
...
- 伺服器端建議賦予 Bot：管理身份組、管理頻道、管理訊息、踢出成員、移動成員（語音）等權限，且 Bot 身份組需高於它要管理的身份組。
...
├── cogs/
│   ├── anti_bot.py          # 誘捕頻道踢人
│   ├── warning_msg.py       # /warning-msg 誘捕頻道警告訊息
